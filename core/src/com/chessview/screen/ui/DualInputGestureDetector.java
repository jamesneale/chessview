...
public class DualInputGestureDetector extends InputAdapter {
	final DualGestureListener listener;
	...
	private Vector2 pinchLocus = new Vector2();
	 ...
	 * maxFlingDelay=0.15f. */
	public DualInputGestureDetector(DualGestureListener listener) {
	...
	public DualInputGestureDetector(float halfTapSquareSize, float tapCountInterval, float longPressDuration, float maxFlingDelay,
		DualGestureListener listener) {
	...
	public boolean touchDown (float x, float y, int pointer, int button) {
		...
		if (pointer == 0) {
			...
			if (Gdx.input.isTouched(1)) {
				...
				inTapSquare = false;
				pinching = true;
				pinchLocus.set(pointer1).lerp(pointer2, 0.5f);
				prevDst = pointer1.dst(pointer2);
	...
	public boolean touchDragged (float x, float y, int pointer) {
		...
		if (pinching) {
			if (listener != null) {
				float curDst = pointer1.dst(pointer2);
				boolean result = listener.zoom(pinchLocus, prevDst - curDst);
				prevDst = curDst;
				return result;
			}
			return false;
	 ...
	 * @author mzechner */
	public static interface DualGestureListener {
		...
		public boolean zoom (Vector2 locus, float amount);
	 ...
	 * @author mzechner */
	public static class GestureAdapter implements DualGestureListener {
		@Override
		public boolean touchDown (float x, float y, int pointer, int button) {
			return false;
		}
		...
		@Override
		public boolean zoom (Vector2 locus, float distance) {
			return false;
		}
